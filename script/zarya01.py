...
MESSAGES = [
    ...
    [
        # через 2 минуты
        '<b>Заря 1 (Каманин):</b> Приступайте к проверке <a href="http://ru.wikipedia.org/wiki/%D0%A1%D0%9A-1_(%D1%81%D0%BA%D0%B0%D1%84%D0%B0%D0%BD%D0%B4%D1%80)">скафандра</a>. Как поняли меня?',
    ],
    [
    ...
    [
        '<b>Заря 1 (Королев):</b> Понял Вас отлично. <a href="https://telegra.ph/Otchet-ot-120461-04-09">Данные Ваши</a> все принял, подтверждаю. '
        'Готовность к старту принял. У нас все идет нормально.',
        # через 2 минуты
        '<b>Заря 1 (Королев):</b> Юрий Алексеевич, я хочу Вам просто напомнить, что после минутной готовности пройдет минуток шесть, прежде чем начнется 🚀. Так что Вы не волнуйтесь.'
    ],
    [
        '<b>Заря 1 (Королев):</b> Ну отлично, прекрасно. После минутной готовности шесть минуток будет, так сказать, всяких дел 😉 Передаю трубку председателю.',
    ...
    [
        ...
        '*далее Юрий дает отсчет для проверки канала связи КВ. А далее в течение двух минут происходит проверка положения тумблера*',
        # in 1 min
    ...
    [
        ...
        '*в течение трёх минут проиходила настройк связи с разной аппаратуры и проверка каналов с Быковым. Быков справлялся о самочувствии Юрия*',
        'Заря 1 (Галай): Проверьте удобства пользования памяткой и видимость кодовой таблицы на "гастрономе". Как поняли?'
        ,'video ./files/video/gagarin.mp4'
    ]
]
BUTTONS = [
    ...
    ['ПРОВЕРИТЬ 👨‍🚀'],
    ['Как меня слышите?'],  # вместо этогоотправляется ГС
    [
    ...
    ['Понял. Я так и знал.'],
    ['Отправить отчёт о состоянии машины'],  # придумать как отправить текст отличный от кнопки
